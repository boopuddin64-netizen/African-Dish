  ...
  return (
    <header id="app-navbar" className="sticky top-0 z-40 bg-[#FAF7F0]/95 dark:bg-[#181512]/95 backdrop-blur-md border-b border-[#EAE4DC] dark:border-stone-800 transition-colors w-full">
      <div className="max-w-7xl mx-auto px-2.5 sm:px-4 lg:px-8">
        ...
        {/* Main Nav Bar Row */}
        <div className="flex items-center justify-between h-14 sm:h-16 lg:h-20 gap-1 sm:gap-3 w-full">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0 min-w-0">
            ...
            >
              <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 rounded-xl bg-[#C85C43] text-white flex items-center justify-center font-bold text-sm sm:text-base lg:text-lg shadow-sm group-hover:bg-[#B44F37] transition-colors shrink-0">
                O
              </div>
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-[#807872] dark:text-stone-400 truncate hidden xl:block">
                  {isMerchant ? 'Restaurant Partner Portal' : (isCourier ? 'Courier Logistics Partner' : 'Curated African Cuisine')}
                </p>
              </div>
            </button>
          </div>

          {/* Middle: Location & Time Context Controls */}
          <div className="flex items-center gap-1 sm:gap-2 min-w-0">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-full bg-white dark:bg-stone-900 border border-[#EAE4DC] dark:border-stone-700 hover:border-[#C85C43]/40 text-xs font-medium text-[#241A17] dark:text-stone-200 shadow-2xs transition-all max-w-[130px] xs:max-w-[150px] sm:max-w-[200px]"
              >
                <MapPin className="w-3.5 h-3.5 text-[#C85C43] shrink-0" />
                <span className="truncate text-[11px] sm:text-xs">
                  {currentLocation.label}
                </span>
                <span className="text-[9px] sm:text-[10px] text-[#807872] dark:text-stone-400 bg-[#FAF7F0] dark:bg-stone-800 px-1 py-0.5 rounded shrink-0 hidden xs:inline">
              ...
              {showLocationMenu && (
                <div 
                  ...
                  className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-72 sm:w-80 bg-white dark:bg-[#1E1B18] rounded-2xl shadow-xl border border-[#EAE4DC] dark:border-stone-800 p-2 z-50 animate-in fade-in zoom-in-95 duration-100 max-w-[calc(100vw-24px)]"
                >
                  ...
                  <div className="py-1 max-h-72 overflow-y-auto">
                    ...
                    {savedLocations.filter(l => l.city === 'Port Harcourt').map(loc => (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className="font-medium truncate">{loc.label}</div>
                          <div className="text-[11px] text-[#807872] dark:text-stone-400 truncate">{loc.address}</div>
                        </div>
                    ...
                    {savedLocations.filter(l => l.city !== 'Port Harcourt').map(loc => (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <div className="font-medium truncate">{loc.label}</div>
                          <div className="text-[11px] text-[#807872] dark:text-stone-400 truncate">{loc.address} ({loc.postcodeOrArea})</div>
                        </div>
            ...
            <div className="relative hidden xs:block">
              <button
                ...
                className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-full bg-white dark:bg-stone-900 border border-[#EAE4DC] dark:border-stone-700 hover:border-[#C85C43]/40 text-xs font-medium text-[#241A17] dark:text-stone-200 shadow-2xs transition-all"
              >
                <Clock className="w-3.5 h-3.5 text-[#5F765A] shrink-0" />
                <span className="capitalize text-[11px] sm:text-xs truncate max-w-[70px] sm:max-w-none">
              ...
              {showTimeMenu && (
                <div 
                  ...
                  className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-56 bg-white dark:bg-[#1E1B18] rounded-2xl shadow-xl border border-[#EAE4DC] dark:border-stone-800 p-2 z-50 animate-in fade-in zoom-in-95 duration-100 max-w-[calc(100vw-24px)]"
                >
                  <div className="px-3 py-2 border-b border-[#F0EAE1] dark:border-stone-800">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[#807872] dark:text-stone-400">
          ...
          {/* Right: Desktop Primary Navigation, Controls & Cart */}
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            
            {/* Primary Desktop Navigation Bar (Only on large screens) */}
            <nav aria-label="Main Navigation" className="hidden lg:flex items-center gap-1 bg-white dark:bg-stone-900 border border-[#EAE4DC] dark:border-stone-800 p-1 rounded-full text-xs font-semibold">
              {isCustomer && (
                <>
                  <button
                    id="nav-home-btn"
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-full border text-[11px] sm:text-xs font-bold shadow-2xs transition-all cursor-pointer ${
                isMerchant
            ...
            >
              ...
              <span className="capitalize text-[10px] sm:text-[11px] font-extrabold hidden xs:inline">
                {userProfile.role === 'restaurant_staff' ? 'Kitchen' : userProfile.role}
              </span>
            ...
            <button
              ...
              className="p-1.5 sm:p-2 rounded-full bg-white dark:bg-stone-900 border border-[#EAE4DC] dark:border-stone-800 text-[#241A17] dark:text-stone-200 hover:border-[#C85C43]/40 shadow-2xs transition-colors"
            ...
            <button
              ...
              className="p-1.5 sm:px-2.5 sm:py-1.5 rounded-full bg-white dark:bg-stone-900 border border-[#EAE4DC] dark:border-stone-800 hover:border-[#C85C43]/40 text-[#241A17] dark:text-stone-200 text-xs font-semibold flex items-center gap-1 shadow-2xs transition-all"
              title="Personal Preferences & Safety Layer"
            >
              <SlidersHorizontal className="w-3.5 h-3.5 text-[#807872] dark:text-stone-400" />
            ...
            {isCustomer && (
              <button
                ...
                className="relative p-1.5 sm:px-3 sm:py-1.5 rounded-full bg-[#C85C43] text-white text-xs font-bold flex items-center gap-1.5 shadow-sm hover:bg-[#B44F37] transition-all"
              >
                <ShoppingBag className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                <span className="hidden md:inline text-xs">Cart</span>
                {totalCartCount > 0 && (
                  <span className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white text-[#C85C43] text-[10px] sm:text-[11px] font-black flex items-center justify-center shadow-xs">
                    {totalCartCount}
                  </span>
                )}
              </button>
        ...
        {/* Mobile / Tablet Primary Navigation Bar (Equal width flex layout with no horizontal overflow) */}
        <nav 
          aria-label="Mobile Navigation"
          className="flex lg:hidden items-center justify-between py-1.5 border-t border-[#EAE4DC] dark:border-stone-800 text-xs font-medium w-full gap-1"
        >
          {isCustomer && (
            <>
              <button
                ...
                className={`flex-1 min-w-0 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl transition-colors ${
                  currentView === 'home' 
                    ? 'bg-[#C85C43] text-white font-bold shadow-2xs' 
                    : 'text-[#807872] dark:text-stone-400 hover:text-[#241A17] dark:hover:text-stone-200'
                }`}
              >
          ...
          {isMerchant && (
            <button
              ...
              className={`flex-1 min-w-0 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl transition-colors ${
                currentView === 'merchant' 
                  ? 'bg-[#5F765A] text-white font-bold shadow-2xs' 
                  : 'text-[#807872] dark:text-stone-400 hover:text-[#241A17] dark:hover:text-stone-200'
              }`}
            >
              <Store className="w-4 h-4 shrink-0" />
              <span className="truncate text-xs">Kitchen</span>
            </button>
          )}

          {isCourier && (
            <button
              id="mobile-nav-courier-btn"
              onClick={() => {
